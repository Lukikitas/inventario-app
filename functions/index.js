exports.proxySirLogin = onCall(
  ...
  async (request) => {
    ...
    // IMPORTANTE: la URL tiene que apuntar a west1
    const sirLoginUrl =
    ...
    try {
      ...
      if (!response.ok) {
        const errorMessage = responseData.error
          ...
          : `Proxy target returned status: ${response.status}`;
      ...
      throw new HttpsError(
        "internal",
        "An unexpected error occurred in the proxy function."
      );
    }
  }
);
...
exports.sirLogin = onRequest({
  region: "southamerica-west1",       // 👈 también acá
  memory: "1GiB",
  timeoutSeconds: 60,
  ...
  if (!user || !password) {
    return res
      .status(400)
      .json({ error: { message: 'The function must be called with "user" and "password" arguments.' } });
  }
